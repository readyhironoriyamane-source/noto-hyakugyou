...
import { ArrowUpRight, Phone, Building2 } from 'lucide-react';
...
export default function IndustriesPage() {
  ...
  // APIから全記事を取得
  const { data: rawArticles, isLoading } = trpc.articles.list.useQuery();

  // DB記事データをIndustry型に変換
  const allIndustries = useMemo(() => {
    if (!rawArticles) return [];
    return mapArticlesToIndustries(rawArticles as any);
  }, [rawArticles]);
      ...
      <main className="pt-[60px]">
        <section className="bg-[#F9F8F4] py-20">
          <div className="max-w-[1140px] mx-auto px-6">
            ...
            {/* 事例カードグリッド */}
            {!isLoading && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-10" style={{ gridTemplateRows: `repeat(${Math.ceil(filteredItems.length / 3)}, auto 1fr auto auto)` }}>
